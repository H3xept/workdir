...
| H1 | Preview `?key=` API token was forwarded to the (untrusted) sandbox upstream and logged. Browser previews also needed a safe way to authenticate subresources and WebSocket upgrades after the first navigation. | `strip_key_query` removes the token before forwarding; logs are redacted. Query-authenticated browser requests set an HttpOnly/Secure preview cookie, and the proxy strips that reserved cookie from both upstream requests and upstream `Set-Cookie` responses. (`api/preview.rs`) |
